...
object test01 {
  def main(args: Array[String]): Unit = {
    ...
    val kafkaDStream: InputDStream[ConsumerRecord[String, String]] = KafkaUtil.getKafkaStream("test_eds", ssc)
    ...
    value.foreachRDD(rdd => {
      ...
      spark.sql(
        """
          |select
          |  t1.*,
          |  case t2.CHECK_STATE when '0' then 'Y' when '1' then 'N1' when '3' then 'N3' when '4' then 'N4' else 'N' END as res_3
